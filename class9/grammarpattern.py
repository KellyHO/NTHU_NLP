...
def nChunk_to_pat(nChunk):
    ...
    for ichunk, chunkInfo in enumerate(nChunk):
        ...
        if ichunk==0 and chunk=='VP':
            if 'be' in lemmaL and 'VBN' in tagL:        res += [ 'V-ed' ]
            elif tags=='VBN':                           res += [ 'V-ed' ]
            else:                                       res += [ 'V' ]
        elif ichunk==0 and chunk=='NP':                 res += [ 'N' ]
        elif ichunk==0 and chunk in ['JP','ADJP']:      res += [ 'ADJ' ]
        elif chunk=='VP':                               res += [ '-ing' if tags == 'VBG' else 'v' ]
        elif lemmas in reversedWords:                   res += [ lemmas ]
        elif chunk == 'NP' and tags == 'VBG':           res += [ '-ing' ]
        elif chunk == 'NP' and lemmas in selfWords:     res += [ 'oneself' ]
        elif chunk == 'NP' and lemmas in whWords:       res += [ 'wh', 'n' ]
        elif chunk == 'NP' and (tags == 'CD' \
                        or lemmas[0] in '0123456789.'): res += [ 'amount' ]
        elif chunk == 'NP' and doubleN(words, lemmas, tags, chunk):
                                                        res += [ 'n n' ]
        elif '"' in lemmas:                             res += [ 'quote' ]
        elif chunk=='ADVP' and lemmas in ordWords:      res += [ 'ord' ]
        elif chunk in ['NP', 'ADJP', 'ADVP', 'VP']:     res += [ defaultMap[chunk] ]
        elif chunk == 'PP':                             res += [ words ]
        elif chunk == 'SBAR' and lemmas == 'that':      res += [ 'that' ]
        elif chunk == 'SBAR':                           res += [ '_' ]
        elif chunk == 'O':                              res += [ '_' ]
        else:                                           res += [ chunk ]
    return ' '.join(res)
...
def getpatterns(nPhrase):
    ...
    def chunk_to_lemma(chunk): return chunk[1] if '_' not in chunk[1] else chunk[1][ chunk[1].rfind('_')+1:]
    def chunk_to_head(chunk): return chunk[1] if '_' not in chunk[1] else chunk[1][ chunk[1].rfind('_')+1:]
    def nChunk_to_words(nChunk): return ' '.join( [nChunk[0][0], '[' ]+[chunk[0] for chunk in nChunk[1:-1] ]+\
                                                  [']', nChunk[-1][0] ])
    def nChunk_to_collocation(nChunk): return ' '.join([ chunk_to_lemma(chunk) for chunk in nChunk ])                   
    ...
    for nChunk, pat in phrases:
        ...
        if VOC and head+'-'+nChunk[1][3][0] not in VOC: continue
        ...
        try:
            ...
            pass
    return results if results else [ ('_', '_', nChunk_to_collocation(nPhrase[1:-1]), '_')]#[ (head, pat, col, ngram) for head, pat, col, ngram in results if head not in ['be-V', 'me-N', 'do-V' ] ]
...
#         pure_sent = '\n'+' '.join([' '.join(x) for x in parse[0] ])
        pure_sent = [' '.join(x) for x in parse[0] ]
        print(pure_sent)
        s = getpatterns(pure_sent)
#         print(s)

#         for start, end in getpatterns(parse[0]):
#             pat = ngram_to_pat(*parse, start, end)
#             if pat: print ('%s\t%s\t%s' %(ngram_to_head(*parse, start, end), pat, ' '.join([' '.join(x) for x in parse[0][start:end] ]) ))
